...
@detect_bp.before_request
def require_inference_service():
    if request.endpoint in {'detect.detect', 'detect.detect_realtime', 'detect.detect_batch'}:
...
def _detect_one_file(file_obj, user):
    ...
    results, result_img = current_app.extensions['model'].detect(file_bytes)
    ...
    if results:
        ...
        average_confidence = max(confidences) if confidences else 0
        ...
        log_action('user_action', f'花卉识别完成：返回 {len(results)} 个候选类别', user.id)
    ...
    return {
        ...
        'message': '识别完成' if results else '未获得花卉候选结果'
    }
...
@detect_bp.route('/detect/realtime', methods=['POST'])
@jwt_required
def detect_realtime():
    payload = request.jwt_payload
    user_id = payload['user_id']
    ...
    try:
        ...
        file_bytes = file_obj.read()
        if not file_bytes:
            return jsonify({'status': 'error', 'message': '文件为空或读取失败'}), 400

        results, _, top1_conf = current_app.extensions['model'].detect_realtime(
            file_bytes,
        ...
        if results:
            ...
            history = DetectionHistory(
                user_id=user.id,
                image_path=f"realtime:{uuid.uuid4().hex}",
                result=result_json,
                confidence=float(top1_conf or 0)
            )
            db.session.add(history)
            db.session.commit()
            history_id = history.id
            log_action('user_action', '实时识别完成', user.id)

        return jsonify({
            'status': 'success',
            'results': results,
            'average_confidence': float(top1_conf) if top1_conf else 0,
            'history_id': history_id,
            'message': '识别完成' if results else '未获得花卉候选结果'
        }), 200
        ...
        db.session.rollback()
        print(f"✗ 实时识别错误: {str(e)}")
        log_action('error', f'实时识别错误: {str(e)}', user_id)
        return jsonify({'status': 'error', 'message': f'处理图像时出错: {str(e)}'}), 500
