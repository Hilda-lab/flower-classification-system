...
def create_app():
    ...
    if app.config['PREVIEW_MODE']:
        ...
        app.logger.warning('本地预览模式：模型未加载，图片识别不可用。')
    else:
        from .services.flower_service import FlowerService
        app.extensions['model'] = FlowerService(
            app.config['FLOWER_MODEL_DIR'], app.config['FLOWER_DEVICE'],
            app.config['FLOWER_NUM_THREADS']
        )
