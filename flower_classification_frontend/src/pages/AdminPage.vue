            ...
            <div class="card h-100">
              <div class="card-body">
                ...
                <div class="d-flex flex-column gap-3">
                  ...
                  <div class="status-indicator">
                    <div class="status-dot" :class="stats.model_ready ? 'online' : 'warning'"></div>
                    <span class="text-muted">花卉 ResNet18 引擎: {{ stats.model_ready === true ? '已加载' : stats.preview_mode ? '未加载（本地预览模式）' : '状态未确认' }}</span>
                  </div>
                  <div class="status-indicator">
                    <div class="status-dot online"></div>
...
const getColorForClass = (className) => {
  const colors = ['gc-bar-flower-1', 'gc-bar-flower-2', 'gc-bar-flower-3', 'gc-bar-flower-4']
  const hash = Array.from(String(className || '')).reduce((sum, char) => sum + char.charCodeAt(0), 0)
