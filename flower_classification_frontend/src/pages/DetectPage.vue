    ...
    <div v-else>
      ...
      />
        ...
        <div class="detect-content">
          <h2 v-reveal id="flower-identification" class="section-title hero-fade-in anim-delay-1">花卉识别</h2>
          <!-- Content remains the same -->
          <div class="alert alert-info align-items-start">
                  ...
                  <div class="card-body">
                    ...
                    <div v-if="result.status === 'success' && result.results && result.results.length > 0">
                      <div class="alert alert-success">
                        <h6><i class="bi bi-check-circle-fill me-2"></i> 识别到 1 个物品</h6>
                      </div>
                      ...
                      <div class="table-responsive">
                        <table class="table table-sm">
                          ...
                          <tbody>
                            <tr v-if="getTopItem(result.results)">
                              <td>
                                <span :class="`category-label ${getCategoryClass(getTopItem(result.results).class_name)}`">
                                  {{ getMajorCategory(getTopItem(result.results).class_name) }}
                                </span>
                              </td>
                              <td>
                                <div v-if="getTopItem(result.results).confidence && getTopItem(result.results).confidence > 0" class="gc-progress-container">
                                  <div
                                    class="gc-progress-bar"
                                    :class="getProgressLevelClass(getTopItem(result.results).confidence)"
                                    :style="{ width: `${(getTopItem(result.results).confidence * 100).toFixed(1)}%` }"
                                  ></div>
                                  <span class="gc-progress-label">{{ (getTopItem(result.results).confidence * 100).toFixed(1) }}%</span>
                                </div>
                                <span v-else class="text-muted small">无法识别</span>
                              </td>
                              <td>
                                {{ getItemName(getTopItem(result.results).class_name) }}
                              </td>
                            </tr>
                          </tbody>
                        </table>
                    ...
                    <div v-else-if="result.status === 'success'" class="alert alert-info">
                      <i class="bi bi-info-circle me-2"></i>未获得花卉候选结果
                    </div>
                  </div>
                </div>
              </div>
...
const getCategoryClass = () => 'recyclable'
const getMajorCategory = () => '花卉候选'
const getItemName = (name) => name || '未知'
